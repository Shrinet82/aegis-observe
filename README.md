...
* [casting.yaml](casting.yaml) — Declarative installation specification adhering strictly to the official SigNoz Foundry schema (`flavor: helm`, `mode: kubernetes`).
* [casting.yaml.lock](casting.yaml.lock) — Resolved deployment lockfile generated via `foundryctl forge -f casting.yaml`.

#### Verified Reproducibility Workflow
Tested & verified using official **SigNoz `foundryctl` v0.2.16**:
```bash
# 1. Install or update foundryctl (if not already installed)
curl -fsSL https://signoz.io/foundry.sh | bash

# 2. Validate casting specification schema
foundryctl gauge -f casting.yaml

# 3. Forge resolved lockfile
foundryctl forge -f casting.yaml

# 4. Provision the SigNoz observability stack in Kubernetes
foundryctl cast -f casting.yaml
```

**Verified Command Execution Output:**
* `foundryctl gauge -f casting.yaml` ➔ `Passed (Exit Code 0)`
* `foundryctl forge -f casting.yaml` ➔ `Lockfile Generated (Exit Code 0)`
* `foundryctl cast -f casting.yaml` ➔ `Release "signoz" deployed (Exit Code 0)`
